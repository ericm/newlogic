...
import { Gates, Wire, GateNode } from '../gates/all';
...
export namespace Saving {
	...
	function deserialize<T extends Gates<T>>(gates: IComponent.GateStatePlecibo[], 
		endNodes: GateNode<any>[], startNodes: GateNode<any>[], ctx: CanvasRenderingContext2D): T[] {
			let output: T[] = [];
			// Get gates and nodes
			for (let gate of gates) {
				let constructed = {} as T;
				constructed.ctx = ctx;
	...
	export function loadState(workspace: Workspace, name?: string): void {
		...
		workspace.gates.and = deserialize(save.gates.and, endNodes, startNodes, workspace.ctx);
		workspace.gates.or = deserialize(save.gates.or, endNodes, startNodes, workspace.ctx);
		workspace.gates.not = deserialize(save.gates.not, endNodes, startNodes, workspace.ctx);
		workspace.gates.switch = deserialize(save.gates.switch, endNodes, startNodes, workspace.ctx);
		workspace.gates.led = deserialize(save.gates.led, endNodes, startNodes, workspace.ctx);
		let all: AnyGate[] = [
			...workspace.gates.and,
			...workspace.gates.or,
			...workspace.gates.not,
